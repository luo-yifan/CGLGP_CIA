...
library(rjags)
setwd("~/Desktop/CGLGP_CIA/L2SWBM")

## Add-in by Dani ##
##### Config setup (workaround) ####
iters = as.integer(as.vector(50000))
halfIters = iters/2
## End add-in ##
##### Set output file(s) name #####
outputname = 'SimplifiedL2S_060221_TestRun_noSFD_2NiagDSets_LogNormals3Eri1Dataset'
...
superiorOutflow = superiorOutflow[,c(1,2,3,4,6)] ## DATASETS ## Let's do IGS (3) and Flow Accounting (4) datasets. 6 is new date format ##
...
superiorPrecip = superiorPrecip[,c(1,2,3,5,13)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 13 is new date format
...
superiorEvap = superiorEvap[,c(1,2,3,5,10)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 10 is new date format
...
superiorRunoff = superiorRunoff[,c(1,2,3,5,10)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 10 is new date format
...
superiorNBS = read.csv('input/SUP_lake_NBS.csv', TRUE, na.strings='-9999.9', skip=4);
superiorNBS[superiorNBS < -5000] = NA
superiorNBS$date <- as.Date(paste(superiorNBS$Year, superiorNBS$Month, "01", sep="-"), format = "%Y-%m-%d")
superiorNBS_Prior = superiorNBS;
superiorNBS = superiorNBS[,c(1,2,3,5,12)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 12 is new date format
...
###### St Clair River Flows (M-H Outflow) - NOT Gauge Data! #####
...
miHuronOutflow = miHuronOutflow[,c(1,2,3,4,6)] ## DATASETS ## Let's do IGS (3) and SFD.ADVM (4) datasets. 6 is new date format
...
## Sample SFD code ##
## miHuronFtGr = read.csv('input/CO-OPS_9014098_met82-18.csv',TRUE)[,c(1,6)] ## Fort Gratiot gauge/stage dataset ##
## miHuronDryD = read.csv('input/CO-OPS_9014087_met82-18.csv',TRUE)[,c(1,6)] ## Dry Dock gauge/stage dataset ##
## miHuronGaugeDate = miHuronFtGr[,1] ## Set date column for consistency with other datasets
## miHuronFtGr$newDate <- strptime(as.character(miHuronFtGr$Date),"%Y/%m/%d") ## New date format ##
## format(miHuronFtGr$newDate,"%Y-%m-%d") ## New date format ##
## miHuronDryD$newDate <- strptime(as.character(miHuronDryD$Date),"%Y/%m/%d") ## New date format ##
## format(miHuronDryD$newDate,"%Y-%m-%d") ## New date format ##
## miHuronSFD.Outflow = cbind(miHuronFtGr[1:2],miHuronDryD[2:3]) ## Combine both gauges into one dataset ##
## names(miHuronSFD.Outflow)[2] <- "FtGrStage" ## Rename column 2 as Fort Gratiot Stage ##
## names(miHuronSFD.Outflow)[3] <- "DryDStage" ## Rename column 3 as Dry Dock Stage ##
...
miHuronPrecip = miHuronPrecip[,c(1,2,3,5,13)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 13 is new date format
...
miHuronEvap = miHuronEvap[,c(1,2,3,5,10)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 10 is new date format
...
miHuronRunoff = miHuronRunoff[,c(1,2,3,5,10)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 10 is new date format
...
miHuronInflow = miHuronInflow[,c(1,2,3,4,6)] ## DATASETS ## Let's do IGS (3) and Flow Accounting (4) datasets. 6 is new date format
...
miHuronNBS = read.csv('input/MHG_lake_NBS.csv', TRUE, na.strings='-9999.9', skip=4);
miHuronNBS[miHuronNBS < -5000] = NA
miHuronNBS$date <- as.Date(paste(miHuronNBS$Year, miHuronNBS$Month, "01", sep="-"), format = "%Y-%m-%d")
miHuronNBS_Prior = miHuronNBS;
miHuronNBS = miHuronNBS[,c(1,2,3,5,12)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 12 is new date format
...
stClairInflow = stClairInflow[,c(1,2,3,4,6)] ## DATASETS ## Let's do IGS (3) and Flow Accounting (4) datasets. 6 is new date format
...
stClairNBS = stClairNBS[,c(1,2,3,5,12)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 12 is new date format
...
###### Detroit River Flows (M-H Outflow) - NOT Gauge Data! #####
...
stClairOutflow = stClairOutflow[,c(1,2,3,4,6)] ## DATASETS ## Let's do IGS (3) and SFD.ADVM (4) datasets. 6 is new date format
...
erieOutflow = erieOutflow[,c(1,2,3,4,5)] ## DATASETS ## Let's do the SFD.ADVM (3) and Coordinated (4, exact same values as SFD.ADVM!) datasets. 5 is new date format
...
eriePrecip = eriePrecip[,c(1,2,3,5,13)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 13 is new date format
...
erieEvap = erieEvap[,c(1,2,3,5,10)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 10 is new date format
...
erieRunoff = erieRunoff[,c(1,2,3,5,10)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 10 is new date format
...
erieInflow = erieInflow[,c(1,2,3,4,6)] ## DATASETS ## Let's do IGS (3) and Flow Accounting (4) datasets. 6 is new date format
...
erieNBS = read.csv('input/ERI_lake_NBS.csv', TRUE, na.strings='-9999.9', skip=4);
erieNBS[erieNBS < -5000] = NA
erieNBS$date <- as.Date(paste(erieNBS$Year, erieNBS$Month, "01", sep="-"), format = "%Y-%m-%d")
erieNBS_Prior = erieNBS;
erieNBS = erieNBS[,c(1,2,3,5,12)] ## DATASETS ## Let's do NOAA GLERL GLM HMD (3) and USACE AHPS (5) datasets. 12 is new date format
...
SupEvapPriorRange0=which(as.numeric(format(as.Date(superiorEvap[,5]),"%Y"))==startPriorYear 
                        & as.numeric(format(as.Date(superiorEvap[,5]),"%m"))==startPriorMonth)
SupEvapPriorRange=which(as.numeric(format(as.Date(superiorEvap[,5]),"%Y"))==endPriorYear
                       & as.numeric(format(as.Date(superiorEvap[,5]),"%m"))==endPriorMonth)
PriorSupEvapData=superiorEvap[SupEvapPriorRange0:SupEvapPriorRange,]
# PriorSupEvapData[PriorSupEvapData==0]=NA
SupNBSPriorRange0=which(as.numeric(format(as.Date(superiorNBS[,5]),"%Y"))==startPriorYear 
                       & as.numeric(format(as.Date(superiorNBS[,5]),"%m"))==startPriorMonth)
SupNBSPriorRange=which(as.numeric(format(as.Date(superiorNBS[,5]),"%Y"))==endPriorYear
                      & as.numeric(format(as.Date(superiorNBS[,5]),"%m"))==endPriorMonth)
PriorSupNBSData=superiorNBS[SupNBSPriorRange0:SupNBSPriorRange,]
# PriorSupNBSData[PriorSupNBSData==0]=NA
SupOutPriorRange0=which(as.numeric(format(as.Date(superiorOutflow[,5]),"%Y"))==startPriorYear 
                       & as.numeric(format(as.Date(superiorOutflow[,5]),"%m"))==startPriorMonth)
SupOutPriorRange=which(as.numeric(format(as.Date(superiorOutflow[,5]),"%Y"))==endPriorYear
                      & as.numeric(format(as.Date(superiorOutflow[,5]),"%m"))==endPriorMonth)
PriorSupOutData=superiorOutflow[SupOutPriorRange0:SupOutPriorRange,]
# PriorSupOutData[PriorSupOutData==0]=NA
SupPrecPriorRange0=which(as.numeric(format(as.Date(superiorPrecip[,5]),"%Y"))==startPriorYear 
                        & as.numeric(format(as.Date(superiorPrecip[,5]),"%m"))==startPriorMonth)
SupPrecPriorRange=which(as.numeric(format(as.Date(superiorPrecip[,5]),"%Y"))==endPriorYear
                       & as.numeric(format(as.Date(superiorPrecip[,5]),"%m"))==endPriorMonth)
...
SupRunPriorRange0=which(as.numeric(format(as.Date(superiorRunoff[,5]),"%Y"))==startPriorYear 
                       & as.numeric(format(as.Date(superiorRunoff[,5]),"%m"))==startPriorMonth)
SupRunPriorRange=which(as.numeric(format(as.Date(superiorRunoff[,5]),"%Y"))==endPriorYear
                      & as.numeric(format(as.Date(superiorRunoff[,5]),"%m"))==endPriorMonth)
...
MHEvapPriorRange0=which(as.numeric(format(as.Date(miHuronEvap[,5]),"%Y"))==startPriorYear 
                       & as.numeric(format(as.Date(miHuronEvap[,5]),"%m"))==startPriorMonth)
MHEvapPriorRange=which(as.numeric(format(as.Date(miHuronEvap[,5]),"%Y"))==endPriorYear
                      & as.numeric(format(as.Date(miHuronEvap[,5]),"%m"))==endPriorMonth)
PriorMHEvapData=miHuronEvap[MHEvapPriorRange0:MHEvapPriorRange,]
# PriorMHEvapData[PriorMHEvapData==0]=NA
MHNBSPriorRange0=which(as.numeric(format(as.Date(miHuronNBS[,5]),"%Y"))==startPriorYear 
                        & as.numeric(format(as.Date(miHuronNBS[,5]),"%m"))==startPriorMonth)
MHNBSPriorRange=which(as.numeric(format(as.Date(miHuronNBS[,5]),"%Y"))==endPriorYear
                       & as.numeric(format(as.Date(miHuronNBS[,5]),"%m"))==endPriorMonth)
PriorMHNBSData=miHuronNBS[MHNBSPriorRange0:MHNBSPriorRange,]
# PriorMHNBSData[PriorMHNBSData==0]=NA
MHOutPriorRange0=which(as.numeric(format(as.Date(miHuronOutflow[,5]),"%Y"))==1950 
                       & as.numeric(format(as.Date(miHuronOutflow[,5]),"%m"))==startPriorMonth)
MHOutPriorRange=which(as.numeric(format(as.Date(miHuronOutflow[,5]),"%Y"))==1985
                      & as.numeric(format(as.Date(miHuronOutflow[,5]),"%m"))==endPriorMonth)
PriorMHOutData=miHuronOutflow[MHOutPriorRange0:MHOutPriorRange,]
# PriorMHOutData[PriorMHOutData==0]=NA
MHPrecPriorRange0=which(as.numeric(format(as.Date(miHuronPrecip[,5]),"%Y"))==startPriorYear 
                        & as.numeric(format(as.Date(miHuronPrecip[,5]),"%m"))==startPriorMonth)
MHPrecPriorRange=which(as.numeric(format(as.Date(miHuronPrecip[,5]),"%Y"))==endPriorYear
                       & as.numeric(format(as.Date(miHuronPrecip[,5]),"%m"))==endPriorMonth)
...
MHRunPriorRange0=which(as.numeric(format(as.Date(miHuronRunoff[,5]),"%Y"))==startPriorYear 
                        & as.numeric(format(as.Date(miHuronRunoff[,5]),"%m"))==startPriorMonth)
MHRunPriorRange=which(as.numeric(format(as.Date(miHuronRunoff[,5]),"%Y"))==endPriorYear
                       & as.numeric(format(as.Date(miHuronRunoff[,5]),"%m"))==endPriorMonth)
PriorMHRunData=miHuronRunoff[MHRunPriorRange0:MHRunPriorRange,]
# PriorMHRunData[PriorMHRunData==0]=NA
MHInflPriorRange0=which(as.numeric(format(as.Date(miHuronInflow[,5]),"%Y"))==startPriorYear 
                       & as.numeric(format(as.Date(miHuronInflow[,5]),"%m"))==startPriorMonth)
MHInflPriorRange=which(as.numeric(format(as.Date(miHuronInflow[,5]),"%Y"))==endPriorYear
                      & as.numeric(format(as.Date(miHuronInflow[,5]),"%m"))==endPriorMonth)
...
LkStClNBSPriorRange0=which(as.numeric(format(as.Date(stClairNBS[,5]),"%Y"))==startPriorYear 
                       & as.numeric(format(as.Date(stClairNBS[,5]),"%m"))==startPriorMonth)
LkStClNBSPriorRange=which(as.numeric(format(as.Date(stClairNBS[,5]),"%Y"))==endPriorYear
                      & as.numeric(format(as.Date(stClairNBS[,5]),"%m"))==endPriorMonth)
PriorLkStClNBSData=stClairNBS[LkStClNBSPriorRange0:LkStClNBSPriorRange,]
# PriorLkStClNBSData[PriorLkStClNBSData==0]=NA
LkStClOutPriorRange0=which(as.numeric(format(as.Date(stClairOutflow[,5]),"%Y"))==1950 
                       & as.numeric(format(as.Date(stClairOutflow[,5]),"%m"))==startPriorMonth)
LkStClOutPriorRange=which(as.numeric(format(as.Date(stClairOutflow[,5]),"%Y"))==1985
                      & as.numeric(format(as.Date(stClairOutflow[,5]),"%m"))==endPriorMonth)
PriorLkStClOutData=stClairOutflow[LkStClOutPriorRange0:LkStClOutPriorRange,]
# PriorLkStClOutData[PriorLkStClOutData==0]=NA
LkStClInflPriorRange0=which(as.numeric(format(as.Date(stClairInflow[,5]),"%Y"))==startPriorYear 
                        & as.numeric(format(as.Date(stClairInflow[,5]),"%m"))==startPriorMonth)
LkStClInflPriorRange=which(as.numeric(format(as.Date(stClairInflow[,5]),"%Y"))==endPriorYear
                       & as.numeric(format(as.Date(stClairInflow[,5]),"%m"))==endPriorMonth)
...
EriEvapPriorRange0=which(as.numeric(format(as.Date(erieEvap[,5]),"%Y"))==startPriorYear 
                        & as.numeric(format(as.Date(erieEvap[,5]),"%m"))==startPriorMonth)
EriEvapPriorRange=which(as.numeric(format(as.Date(erieEvap[,5]),"%Y"))==endPriorYear
                       & as.numeric(format(as.Date(erieEvap[,5]),"%m"))==endPriorMonth)
PriorEriEvapData=erieEvap[EriEvapPriorRange0:EriEvapPriorRange,]
# PriorEriEvapData[PriorEriEvapData==0]=NA
EriNBSPriorRange0=which(as.numeric(format(as.Date(erieNBS[,5]),"%Y"))==startPriorYear 
                       & as.numeric(format(as.Date(erieNBS[,5]),"%m"))==startPriorMonth)
EriNBSPriorRange=which(as.numeric(format(as.Date(erieNBS[,5]),"%Y"))==endPriorYear
                      & as.numeric(format(as.Date(erieNBS[,5]),"%m"))==endPriorMonth)
PriorEriNBSData=erieNBS[EriNBSPriorRange0:EriNBSPriorRange,]
# PriorEriNBSData[PriorEriNBSData==0]=NA
...
EriPrecPriorRange0=which(as.numeric(format(as.Date(eriePrecip[,5]),"%Y"))==startPriorYear 
                        & as.numeric(format(as.Date(eriePrecip[,5]),"%m"))==startPriorMonth)
EriPrecPriorRange=which(as.numeric(format(as.Date(eriePrecip[,5]),"%Y"))==endPriorYear
                       & as.numeric(format(as.Date(eriePrecip[,5]),"%m"))==endPriorMonth)
...
EriRunPriorRange0=which(as.numeric(format(as.Date(erieRunoff[,5]),"%Y"))==startPriorYear 
                       & as.numeric(format(as.Date(erieRunoff[,5]),"%m"))==startPriorMonth)
EriRunPriorRange=which(as.numeric(format(as.Date(erieRunoff[,5]),"%Y"))==endPriorYear
                      & as.numeric(format(as.Date(erieRunoff[,5]),"%m"))==endPriorMonth)
PriorEriRunData=erieRunoff[EriRunPriorRange0:EriRunPriorRange,]
# PriorEriRunData[PriorEriRunData==0]=NA
EriInflPriorRange0=which(as.numeric(format(as.Date(erieInflow[,5]),"%Y"))==startPriorYear 
                        & as.numeric(format(as.Date(erieInflow[,5]),"%m"))==startPriorMonth)
EriInflPriorRange=which(as.numeric(format(as.Date(erieInflow[,5]),"%Y"))==endPriorYear
                       & as.numeric(format(as.Date(erieInflow[,5]),"%m"))==endPriorMonth)
...
for (i in 1:12){
  ###### Precipitation ######
  ## Superior ##
  datPrecipSup      =PriorSupPrecData[as.numeric(format(as.Date(PriorSupPrecData[,5]),"%m"))==i,Precipitation.Column.Number]
  ...
  ## Michigan-Huron ##
  datPrecipMH      =PriorMHPrecData[as.numeric(format(as.Date(PriorMHPrecData[,5]),"%m"))==i,Precipitation.Column.Number]
  ...
  ## Erie ##
  datPrecipEri     =PriorEriPrecData[as.numeric(format(as.Date(PriorEriPrecData[,5]),"%m"))==i,Precipitation.Column.Number]
  ...
  ###### Evaporation ######
  ## Superior ##
  EvapDatSup         =PriorSupEvapData[as.numeric(format(as.Date(PriorSupEvapData[,5]),"%m"))==i,Evap.Column.Number]
  ...
  ## Michigan-Huron ##
  EvapDatMH         =PriorMHEvapData[as.numeric(format(as.Date(PriorMHEvapData[,5]),"%m"))==i,Evap.Column.Number]
  ...
  ## Erie ##
  EvapDatEri         =PriorEriEvapData[as.numeric(format(as.Date(PriorEriEvapData[,5]),"%m"))==i,Evap.Column.Number]
  ...
  ###### Runoff ######
  ## Superior ##
  logRunoffDatSup         =log(PriorSupRunData[as.numeric(format(as.Date(PriorSupRunData[,5]),"%m"))==i,Runoff.Column.Number])
  ...
  ## Michigan-Huron ##
  logRunoffDatMH         =log(PriorMHRunData[as.numeric(format(as.Date(PriorMHRunData[,5]),"%m"))==i,Runoff.Column.Number])
  ...
  ## Erie ##
  logRunoffDatEri         =log(PriorEriRunData[as.numeric(format(as.Date(PriorEriRunData[,5]),"%m"))==i,Runoff.Column.Number])
  ...
  ## LAKE St. Clair ##
  NBSDatLkStCl           =PriorLkStClNBSData[as.numeric(format(as.Date(PriorLkStClNBSData[,5]),"%m"))==i,NBS.Column.Number]
  ...
  ## Superior ##
  OutflowDatSup         =PriorSupOutData[as.numeric(format(as.Date(PriorSupOutData[,5]),"%m"))==i,Outflow.Column.Number]
  ...
  ## Michigan-Huron ##
  OutflowDatMH          =PriorMHOutData[as.numeric(format(as.Date(PriorMHOutData[,5]),"%m"))==i,Outflow.Column.Number]
  ...
  ## LAKE St. Clair ##
  OutflowDatLkStCl          =PriorLkStClOutData[as.numeric(format(as.Date(PriorLkStClOutData[,5]),"%m"))==i,Outflow.Column.Number]
  ...
  InflowDatMH         =PriorMHInflData[as.numeric(format(as.Date(PriorMHInflData[,5]),"%m"))==i,Inflow.Column.Number]
  ...
  ## LAKE St. Clair ##
  InflowDatLkStCl         =PriorLkStClInflData[as.numeric(format(as.Date(PriorLkStClInflData[,5]),"%m"))==i,Inflow.Column.Number]
  ...
  ## Erie ##
  InflowDatEri         =PriorEriInflData[as.numeric(format(as.Date(PriorEriInflData[,5]),"%m"))==i,Inflow.Column.Number]
...
yLkStClOut1   =as.numeric(stClairOutflow[PosteriorRange,3]) ## Detroit IGS ##
yLkStClOut2   =as.numeric(stClairOutflow[PosteriorRange,4]) ## Detroit SFD.ADVM ##
...
inputDataCoreJAGS=list(
  ...
  "yLkStClOutflow1"       =yLkStClOut1,
  "yLkStClOutflow2"       =yLkStClOut2,
...
ParamsToMonitor=c(
  ...
  "yLkStClOutflow1Precision",
  "yLkStClOutflow2Precision",
  "yLkStClNBS1PP",
  "yLkStClNBS2PP",
  "yLkStClOutflow1PP",
  "yLkStClOutflow2PP",
...
jMod = jags.model(file = "SimplifiedL2S_jagsModel.jags.r", data = inputDataCoreJAGS, n.chains = 3) 
...
plot(jSample[[1]][,"yEriOutflow2Mean[1]"])
